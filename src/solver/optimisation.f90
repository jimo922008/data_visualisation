MODULE optimisation
    ...
    real(kind=dp)             :: growth_tol_coeff=2.0_dp
...
contains
    ...
    subroutine tpsd(threshold, maxsteps)
        ...
        call initialize_variables(cost, gradient_norm, running_gradient_norm, i, position_vector)

        call start_timer()  

        do while((((running_gradient_norm > log10(threshold*growth_tol_coeff) .or. (i < 100+nexag))) .and. (i < maxsteps)))
    ...
    subroutine initialize_variables(cost, gradient_norm, running_gradient_norm, i, position_vector)
        implicit none
        real(kind=dp), intent(out) :: cost, gradient_norm, running_gradient_norm
        ...
        cost = calculating_cost_zero(pij)
    ...
    subroutine loss_gradient_vectorisation(gradient_vector, cost, exageration, growth_switch)
        ...
        do i=1,reduced_number_points
            ...
            call loss_gradient_core(i, rij2_vector, pos_matrix, gradient_matrix, cost, growth_switch)
    ...
    subroutine loss_gradient_core (i, rij2_vector, pos_matrix, gradient_matrix, cost, growth_switch)
